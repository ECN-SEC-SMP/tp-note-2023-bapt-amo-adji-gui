...
void Board::addExternalWalls() {
  for (int i = 0; i < BOARD_SIZE; i++) {
    ...
    boxBoard[BOARD_SIZE - 1][i].setWall(
        eBottom);                  // x = BOARD_SIZE, y = i ==> Last row
    boxBoard[i][0].setWall(eLeft); // x = i, y = 0          ==> First column
    boxBoard[i][BOARD_SIZE - 1].setWall(
        eRight); // x = i, y = BOARD_SIZE ==> Last column
  }
}

void Board::addCenterSquare() {
  addCorner(boxBoard, ((BOARD_SIZE / 2) - 1), ((BOARD_SIZE / 2) - 1), eTL,
            true);
...
void Board::addInsideWalls() {
  ...
  while (j < 4) {
    // Creates two value [val1, val2] ∈ [0, BOARD_SIZE - 1 - 1]² = [0,
    // BOARD_SIZE - 2]²
    do {
    ...
    default:
      std::runtime_error(
          "[Board, \"addInsideWalls\"] j value should not go above 3");
      break;
    }
    j++;
  }
